...
def obtenerPlan(evento):
    ...
    # En este diccionario guardaremos toda la información del plan que seanecesaria para realizar las predicciones
...
def actualizarVectores(evento):
    # Si el evento es de comienzo de viaje, añadimos el tiempo estimado deviaje al vector
    if (evento["eventType"] in ["Truck departed", "Truck departed todepot"]):

        vectores[(evento["simulationId"],evento["truckId"])]["vector"] =np.array(vectores[(evento["simulationId"],evento["truckId"])]
        ["tiemposEstimados"].pop(0))
    # Si el evento es de comienzo de entrega, añadimos el id del camión alvector
    elif (evento["eventType"] == "Truck started delivering"):
        vectores[(evento["simulationId"],evento["truckId"])]["vector"] =np.array(evento["truckId"])
        
    return (vectores,'se ha actualizado el vector de eventos ')
  

def prediccionDeTiempoDeViaje(evento):
    vector = vectores[(evento["simulationId"],evento["truckId"])]
    ["vector"].reshape(-1, 1)
    prediccion = modelo_tiempo_viaje.predict(vector)[0]
    return (prediccion,'esto es una prediccion de tiempo de viaje')
...
def prediccionDeTiempoDeEntrega(evento):
    ...
    return (prediccion,'esto es una prediccion de tiempo de entrega')
