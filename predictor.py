import pickle
from flask import Flask, jsonify, request , abort
import numpy as np
import json

app = Flask(__name__)


# Load the trained scikit-learn models stored in pickle format
with open('C:/Users/anny0/Downloads/GitAPSE/APSELogisticSimulator-main1/src/models/travelModel.pkl', 'rb') as f:
    modelo_tiempo_viaje = pickle.load(f)

with open('C:/Users/anny0/Downloads/GitAPSE/APSELogisticSimulator-main1/src/models/deliveryModel.pkl', 'rb') as f:
    modelo_tiempo_entrega = pickle.load(f)

with open('C:/Users/anny0/Downloads/GitAPSE/APSELogisticSimulator-main1/src/models/le.pkl', 'rb') as f:
    labelEncoder = pickle.load(f)



# Endpoint for route prediction model
# Input is a json object with attribute time
@app.route('/predict_eta', methods=['POST','GET'])
def predict_eta():
    data = np.array(request.get_json()['time'])
    
    prediccion = modelo_tiempo_viaje.predict(data) 

    
    # Return the prediction as a JSON response
    return jsonify({'prediction': prediccion[0]})
    

# Endpoint for load delivery endpoint.
# Input is a json object with attributes truckId and time
@app.route('/predict_delivery', methods=['POST','GET'])

def predict_delivery():
    data_time = np.array(request.get_json()['time'])
    data_truckid = np.array(request.get_json()['truckId'])
    
    prediccion = modelo_tiempo_entrega.predict(np.array([data_time,data_truckid]))
    # Return the prediction as a JSON response  
    return jsonify({'prediction': prediccion[0]})



if __name__ == '__main__':
    app.run(debug=True, port =7777)